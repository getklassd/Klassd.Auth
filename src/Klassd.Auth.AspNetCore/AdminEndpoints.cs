...
public static class AdminEndpoints
{
        ...
        this IEndpointRouteBuilder app, string basePath = "/auth/admin", string? authorizationPolicy = null)
    {
        ...
        g.MapGet("/users", async (IUserAccountService accounts, IRolesService roles, CancellationToken ct) =>
        {
        ...
        g.MapGet("/users/{id}", async (string id, IUserAccountService accounts, IRolesService roles, CancellationToken ct) =>
        {
            var user = await accounts.GetByIdAsync(id, ct);
            return user is null ? Results.NotFound() : Results.Ok(await ToSummary(user, roles, ct));
        });

        g.MapPost("/users", async (CreateUser req, IUserAccountService accounts, IRolesService roles, CancellationToken ct) =>
        {
            try
            {
        ...
        g.MapPost("/users/{id}/disable", async (string id, SetDisabled req, IUserAccountService accounts, CancellationToken ct) =>
            await accounts.SetDisabledAsync(id, req.Disabled, ct) ? Results.NoContent() : Results.NotFound());

        g.MapPost("/users/{id}/reset-password", async (string id, ResetPassword req, IUserAccountService accounts, CancellationToken ct) =>
            await accounts.ResetPasswordAsync(id, req.NewPassword, ct) ? Results.NoContent() : Results.NotFound());

        g.MapGet("/users/{id}/roles", async (string id, IRolesService roles, CancellationToken ct) =>
            Results.Ok(await roles.GetRolesAsync(id, ct)));

        g.MapPut("/users/{id}/roles", async (string id, SetRoles req, IUserAccountService accounts, IRolesService roles, CancellationToken ct) =>
        {
    ...
    private static async Task<UserSummary> ToSummary(User u, IRolesService roles, CancellationToken ct) => new(
